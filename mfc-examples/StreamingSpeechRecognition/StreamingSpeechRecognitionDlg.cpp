	...
	if (!started_)
	{
		...
		PaError err =
			Pa_OpenStream(&pa_stream_, &param, nullptr, /* &outputParameters, */
				...
				RecordCallback, this);
		if (err != paNoError) {
			AppendLineToMultilineEditCtrl(std::string("PortAudio error: ") + Pa_GetErrorText(err));
...
void CStreamingSpeechRecognitionDlg::InitMicrophone() {
	...
	AppendLineToMultilineEditCtrl(std::string("Selected device ") + Pa_GetDeviceInfo(default_device)->name);
}
...
#if 1
	std::string prefix = "C:/Users/fangjun/source/repos/MFCApplication2/x64/Release";
...
// see https://stackoverflow.com/questions/7153935/how-to-convert-utf-8-stdstring-to-utf-16-stdwstring
...
void CStreamingSpeechRecognitionDlg::AppendTextToEditCtrl(const std::string& s)
{
...
void CStreamingSpeechRecognitionDlg::AppendLineToMultilineEditCtrl(const std::string& s)
{
	AppendTextToEditCtrl("\r\n" + s);
}

static std::string Cat(const std::vector<std::string>& results, const std::string& s)
{
