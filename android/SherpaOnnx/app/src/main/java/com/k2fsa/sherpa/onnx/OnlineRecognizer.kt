...
fun getModelConfig(type: Int): OnlineModelConfig? {
    when (type) {
        ...
        2000 -> OnlineModelConfig(
            transducer = OnlineTransducerModelConfig(
                encoder = "zipformer_vi/encoder-epoch-20-avg-10.onnx",
                decoder = "zipformer_vi/decoder-epoch-20-avg-10.onnx",
                joiner  = "zipformer_vi/joiner-epoch-20-avg-10.onnx",
            ),
            // Không dùng Paraformer nên để trống
            paraformer = OnlineParaformerModelConfig(
                encoder = "",
                decoder = "",
            ),
            // Dùng luôn bpe.model làm tokens
            tokens = "zipformer_vi/bpe.model",
            numThreads = 4,
            provider = "cpu",
            debug = false,          // <--- CHỈNH CHỖ NÀY
            modelType = "zipformer" // (tuỳ chọn, nhưng nên set cho rõ)
        )
