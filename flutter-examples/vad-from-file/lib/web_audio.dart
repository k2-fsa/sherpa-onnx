...
/// Save WAV bytes with a "Save As" dialog on web.
/// Uses the File System Access API (Chromium) with fallback to download.
Future<void> saveWavBytesWithDialog(Uint8List wavBytes, String filename) async {
  globalContext['_sherpaSaveBytes'] = wavBytes.toJS;
  globalContext['_sherpaSaveFilename'] = filename.toJS;

  final eval = globalContext.getProperty('eval'.toJS) as JSFunction;
  eval.callAsFunction(null, '''
    (async function() {
      var bytes = window._sherpaSaveBytes;
      var name = window._sherpaSaveFilename || 'audio.wav';
      var blob = new Blob([bytes], {type: 'audio/wav'});
      ...
      if (typeof window.showSaveFilePicker === 'function') {
        try {
          ...
          window._sherpaSaveBytes = null;
          window._sherpaSaveFilename = null;
          return;
        } catch (e) {
