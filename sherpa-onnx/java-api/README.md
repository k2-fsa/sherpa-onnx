...
| File | Description |
|---|---|
| `target/sherpa-onnx-jvm-1.13.7.jar` | JVM API jar (classes only) |
...
javac -cp sherpa-onnx/java-api/target/sherpa-onnx-jvm-1.13.7.jar YourApp.java

# Run your code (with JNI library path)
java -Djava.library.path=/path/to/jni/libs -cp sherpa-onnx/java-api/target/sherpa-onnx-jvm-1.13.7.jar YourApp
```
...
The current version is `1.13.7` (defined in `pom.xml`).
