...
2. 将解压后的 `libonnxruntime.so` 文件复制到系统库目录，并创建软链接：
   ...
   ```bash
   sudo cp onnxruntime-linux-x64-1.27.0/lib/libonnxruntime.so* /usr/local/lib/
   sudo ln -sf /usr/local/lib/libonnxruntime.so.1.27.0 /usr/local/lib/libonnxruntime.so
   ```
3. 更新共享库缓存并验证安装：
...
1. 下载 macOS ARM64 版本二进制包：
   ...
   ```bash
   wget https://github.com/microsoft/onnxruntime/releases/download/v1.27.0/onnxruntime-osx-arm64-1.27.0.tgz
   tar -xzf onnxruntime-osx-arm64-1.27.0.tgz
   ```
2. 将 `libonnxruntime.1.27.0.dylib` 复制到 `/usr/local/lib`：

   ```bash
   sudo cp onnxruntime-osx-arm64-1.27.0/lib/libonnxruntime.1.27.0.dylib /usr/local/lib/
   ```
