...
#ifndef SHERPA_ONNX_CSRC_AXCL_ONLINE_RECOGNIZER_TRANSDUCER_AXCL_IMPL_H_
#define SHERPA_ONNX_CSRC_AXCL_ONLINE_RECOGNIZER_TRANSDUCER_AXCL_IMPL_H_
...
OnlineRecognizerResult Convert(const OnlineTransducerDecoderResultAxcl &src,
...
class OnlineRecognizerTransducerAxclImpl : public OnlineRecognizerImpl {
 public:
  explicit OnlineRecognizerTransducerAxclImpl(
      const OnlineRecognizerConfig &config)
      : OnlineRecognizerImpl(config),
        config_(config),
        endpoint_(config_.endpoint_config),
        model_(std::make_unique<OnlineZipformerTransducerModelAxcl>(
            config.model_config)) {
    ...
    if (config.decoding_method == "greedy_search") {
      decoder_ = std::make_unique<OnlineTransducerGreedySearchDecoderAxcl>(
          model_.get(), unk_id_);
    } else if (config.decoding_method == "modified_beam_search") {
      decoder_ =
          std::make_unique<OnlineTransducerModifiedBeamSearchDecoderAxcl>(
  ...
  std::unique_ptr<OnlineStream> CreateStream() const override {
    auto stream = std::make_unique<OnlineStreamAxcl>(config_.feat_config);
  ...
  std::unique_ptr<OnlineStream> CreateStream(
    ...
    SHERPA_ONNX_LOGE("Hotwords for AXCL is not supported now.");
    return CreateStream();
  }
  ...
  void DecodeStreams(OnlineStream **ss, int32_t n) const override {
    for (int32_t i = 0; i < n; ++i) {
      DecodeStream(reinterpret_cast<OnlineStreamAxcl *>(ss[i]));
    }
  }

  OnlineRecognizerResult GetResult(OnlineStream *s) const override {
    OnlineTransducerDecoderResultAxcl decoder_result =
        reinterpret_cast<OnlineStreamAxcl *>(s)->GetZipformerResult();
  ...
  bool IsEndpoint(OnlineStream *s) const override {
    ...
    int32_t trailing_silence_frames = reinterpret_cast<OnlineStreamAxcl *>(s)
                                          ->GetZipformerResult()
                                          .num_trailing_blanks *
                                      4;
  ...
  void Reset(OnlineStream *s) const override {
    ...
    // reinterpret_cast<OnlineStreamAxcl*>(s)->SetZipformerEncoderStates(model_->GetEncoderInitStates());
    auto r = decoder_->GetEmptyResult();
    auto last_result =
        reinterpret_cast<OnlineStreamAxcl *>(s)->GetZipformerResult();
    ...
    reinterpret_cast<OnlineStreamAxcl *>(s)->SetZipformerResult(std::move(r));
 ...
 private:
  void DecodeStream(OnlineStreamAxcl *s) const {
 ...
 private:
  ...
  Endpoint endpoint_;
  int32_t unk_id_ = -1;
  std::unique_ptr<OnlineZipformerTransducerModelAxcl> model_;
  std::unique_ptr<OnlineTransducerDecoderAxcl> decoder_;
};

}  // namespace sherpa_onnx

#endif  // SHERPA_ONNX_CSRC_AXCL_ONLINE_RECOGNIZER_TRANSDUCER_AXCL_IMPL_H_
