...
int test1() {
  ...
  int32_t feat_dim = 560;

  features.resize(83 * 512);
  feat_dim = 512;

  int32_t num_frames = features.size() / feat_dim;
  printf("features: %dx%d, %d, %d\n", num_frames, feat_dim,
         feat_dim * num_frames, (int)features.size());
  ...
  // fprintf(stderr,
  //         "loading model: %d, model "
  //         "id: %u %x\n",
  //         (int)ret, model_id, model_id);

  uint32_t model_id2 = 0;
  ret = aclmdlLoadFromFileWithMem("./model.om", &model_id2, model_work_ptr,
                                  work_size, weight_ptr, weight_size);
  CHECK(ret);
  ...
  ret = aclmdlGetDesc(model_desc, model_id2);
  CHECK(ret);
  ...
  size_t index = 0;
  ret = aclmdlGetInputIndexByName(model_desc, ACL_DYNAMIC_TENSOR_NAME, &index);
  CHECK(ret);
  ...
  ret = aclmdlSetInputDynamicDims(model_id2, input_dataset, 0, &dims);
  CHECK(ret);
  printf(
