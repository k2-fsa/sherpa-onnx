 ...
 public:
  explicit OfflineTtsZipvoiceImpl(const OfflineTtsConfig &config)
    ...
    GeneratedAudio Generate(const std::string &text,
      ...
      if (prompt_token_ids.empty() || (prompt_token_ids.size() == 1 &&
   ...
   private:
    ...
    GeneratedAudio Process(const std::vector<int64_t> &tokens,
      ...
      std::array<int64_t, 2> tokens_shape = {
      ...
      Ort::Value mel = model_->Run(tokens_tensor, prompt_tokens_tensor,
                                   prompt_features_tensor, speed, num_step);

      // Assume mel_shape = {1, T, C}
      std::vector<int64_t> mel_shape =
          mel.GetTensorTypeAndShapeInfo().GetShape();
