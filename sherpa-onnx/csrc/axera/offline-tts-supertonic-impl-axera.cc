...
OfflineTtsSupertonicImplAxera::OfflineTtsSupertonicImplAxera(
    const OfflineTtsConfig &config)
    : config_(config),
      model_(std::make_unique<OfflineTtsSupertonicModelAxera>(
          config.model)),
...
int32_t OfflineTtsSupertonicImplAxera::SampleRate() const {
  return model_->GetSampleRate();
}

GeneratedAudio OfflineTtsSupertonicImplAxera::Generate(
...
GeneratedAudio OfflineTtsSupertonicImplAxera::Process(
...
GeneratedAudio
OfflineTtsSupertonicImplAxera::ProcessChunksAndConcatenate(
...
void OfflineTtsSupertonicImplAxera::InitVoiceStyle(
...
OfflineTtsSupertonicImplAxera::StyleSliceView
OfflineTtsSupertonicImplAxera::GetStyleSliceForSid(int64_t sid) const {
  StyleSliceView out;
  int32_t s = 0;
  if (num_speakers_ != 1) {
...
#if __ANDROID_API__ >= 9
template OfflineTtsSupertonicImplAxera::OfflineTtsSupertonicImplAxera(
