...
OnlineRecognizerResult Convert(const OnlineTransducerDecoderResultAxera &src,
...
class OnlineRecognizerTransducerAxeraImpl : public OnlineRecognizerImpl {
 public:
  explicit OnlineRecognizerTransducerAxeraImpl(
      const OnlineRecognizerConfig &config)
      : OnlineRecognizerImpl(config),
        config_(config),
        endpoint_(config_.endpoint_config),
        model_(std::make_unique<OnlineZipformerTransducerModelAxera>(
            config.model_config)) {
    ...
    if (config.decoding_method == "greedy_search") {
      decoder_ = std::make_unique<OnlineTransducerGreedySearchDecoderAxera>(
          model_.get(), unk_id_);
    } else if (config.decoding_method == "modified_beam_search") {
      decoder_ =
          std::make_unique<OnlineTransducerModifiedBeamSearchDecoderAxera>(
  ...
  std::unique_ptr<OnlineStream> CreateStream() const override {
    auto stream = std::make_unique<OnlineStreamAxera>(config_.feat_config);
  ...
  void DecodeStreams(OnlineStream **ss, int32_t n) const override {
    for (int32_t i = 0; i < n; ++i) {
      DecodeStream(reinterpret_cast<OnlineStreamAxera *>(ss[i]));
    }
  }

  OnlineRecognizerResult GetResult(OnlineStream *s) const override {
    OnlineTransducerDecoderResultAxera decoder_result =
  ...
  bool IsEndpoint(OnlineStream *s) const override {
    ...
    int32_t trailing_silence_frames = reinterpret_cast<OnlineStreamAxera *>(s)
                                          ->GetZipformerResult()
                                          .num_trailing_blanks *
                                      4;
 ...
 private:
  void DecodeStream(OnlineStreamAxera *s) const {
 ...
 private:
  ...
  Endpoint endpoint_;
  int32_t unk_id_ = -1;
  std::unique_ptr<OnlineZipformerTransducerModelAxera> model_;
  std::unique_ptr<OnlineTransducerDecoderAxera> decoder_;
};
