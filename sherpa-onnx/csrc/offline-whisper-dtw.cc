...
bool StartsWithSpace(const std::string &s) {
  ...
  // Check for regular ASCII space
  if (s[0] == ' ') {
    return true;
  }

  // Check for Whisper's special space token (U+0120 = Ġ in GPT tokenizer)
  // This is encoded as 0xC4 0xA0 in UTF-8
  if (s.size() >= 2) {
...
std::vector<WhisperWordBoundary> ComputeWordBoundaries(
  ...
  std::vector<WhisperWordBoundary> words;

  if (tokens.empty()) {
    return words;
  }

  WhisperWordBoundary current_word;
  current_word.start_token = 0;
  current_word.end_token = 0;
  current_word.word = "";
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    // Get first character to check for word boundary
    uint32_t first_char = GetFirstCodepoint(token);
    bool is_cjk = IsCJKCharacter(first_char);
    bool starts_with_space = StartsWithSpace(token);

    if (is_cjk) {
      // For CJK, each character is a separate word
      // First, save any pending word
      if (!current_word.word.empty()) {
        current_word.end_token = static_cast<int32_t>(i);
        words.push_back(current_word);
      }

      // Each CJK token is its own word
      current_word.word = token;
      current_word.start_token = static_cast<int32_t>(i);
      current_word.end_token = static_cast<int32_t>(i + 1);
      words.push_back(current_word);

      // Reset for next word
      current_word.word = "";
      current_word.start_token = static_cast<int32_t>(i + 1);
    } else if (starts_with_space || i == 0) {
      // New word starts (space prefix or first token)
      if (!current_word.word.empty()) {
        current_word.end_token = static_cast<int32_t>(i);
        words.push_back(current_word);
      }
      ...
      if (starts_with_space && token.size() > 1) {
        // Skip the space character(s)
        size_t start = 1;
        if (token.size() >= 2 && static_cast<uint8_t>(token[0]) == 0xC4 &&
            static_cast<uint8_t>(token[1]) == 0xA0) {
          start = 2;  // Skip Ġ
        }
        current_word.word = token.substr(start);
  ...
  if (!current_word.word.empty()) {
    current_word.end_token = static_cast<int32_t>(tokens.size());
    words.push_back(current_word);
  }
