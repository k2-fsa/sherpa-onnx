...
class StreamingDft {
 public:
  explicit StreamingDft(int32_t n_fft)
      : n_fft_(n_fft),
        num_bins_(n_fft / 2 + 1),
        cos_table_(num_bins_ * n_fft_),
        sin_table_(num_bins_ * n_fft_) {
    constexpr double kPi = 3.14159265358979323846;
    for (int32_t k = 0; k != num_bins_; ++k) {
      for (int32_t n = 0; n != n_fft_; ++n) {
        double angle = 2.0 * kPi * k * n / n_fft_;
        cos_table_[k * n_fft_ + n] = std::cos(angle);
        sin_table_[k * n_fft_ + n] = std::sin(angle);
      }
    }
  }
  ...
  void Forward(const float *input, float *output) const {
    for (int32_t k = 0; k != num_bins_; ++k) {
      double real = 0;
      double imag = 0;
      const double *p_cos = cos_table_.data() + k * n_fft_;
      const double *p_sin = sin_table_.data() + k * n_fft_;
      for (int32_t n = 0; n != n_fft_; ++n) {
        double v = input[n];
        real += v * p_cos[n];
        imag -= v * p_sin[n];
  ...
  void Inverse(const float *input, float *output) const {
    for (int32_t n = 0; n != n_fft_; ++n) {
      ...
      for (int32_t k = 1; k != num_bins_ - 1; ++k) {
        ...
        int32_t offset = k * n_fft_ + n;
        sum += 2.0 *
               (real * cos_table_[offset] - imag * sin_table_[offset]);
      }
...
class OnlineSpeechDenoiser::Impl {
 public:
  explicit Impl(const OnlineSpeechDenoiserConfig &config)
      : config_(config),
        model_(config.model),
        meta_(model_.GetMetaData()),
        fft_(meta_.n_fft),
        memory_info_(
            Ort::MemoryInfo::CreateCpu(OrtDeviceAllocator, OrtMemTypeDefault)),
        window_(MakeVorbisWindow(meta_.window_length)),
        analysis_buffer_(meta_.window_length),
        overlap_add_buffer_(meta_.window_length),
        fft_input_(meta_.window_length),
        fft_output_(meta_.spec_shape[2] * meta_.spec_shape[3]),
        ifft_output_(meta_.window_length),
        zero_hop_(meta_.hop_length),
        state_(model_.GetInitState()) {
    Init();
  }

  template <typename Manager>
  Impl(Manager *mgr, const OnlineSpeechDenoiserConfig &config)
      : config_(config),
  ...
  DenoisedAudio Run(const float *samples, int32_t n, int32_t sample_rate) {
    ...
    DenoisedAudio ans;
    ans.sample_rate = meta_.sample_rate;
  ...
  DenoisedAudio Flush() {
    ...
    if (!pending_input_.empty()) {
      std::vector<float> padded(meta_.hop_length, 0.0f);
  ...
  int32_t GetSampleRate() const { return meta_.sample_rate; }

  int32_t GetFrameShiftInSamples() const { return meta_.hop_length; }
 ...
 private:
  void Init() {
    ...
    if (meta_.spec_shape.size() != 4 || meta_.spec_shape[0] != 1 ||
        meta_.spec_shape[1] != 1 || meta_.spec_shape[3] != 2) {
      SHERPA_ONNX_LOGE(
  ...
  void CreateResamplerIfNeeded() {
    if (input_sample_rate_ == meta_.sample_rate) {
      return;
    }
    ...
    SHERPA_ONNX_LOGE(
        ...
        input_sample_rate_, meta_.sample_rate);

    float min_freq = std::min<int32_t>(input_sample_rate_, meta_.sample_rate);
    float lowpass_cutoff = 0.99f * 0.5f * min_freq;
    int32_t lowpass_filter_width = 6;
    resampler_ = std::make_unique<LinearResample>(
        input_sample_rate_, meta_.sample_rate, lowpass_cutoff,
  ...
  std::vector<float> ProcessPending() {
    ...
    size_t consumed = 0;
    while (pending_input_.size() - consumed >=
           static_cast<size_t>(meta_.hop_length)) {
      ProcessHop(pending_input_.data() + consumed, &ans);
      consumed += meta_.hop_length;
    }
  ...
  void ProcessHop(const float *hop, std::vector<float> *output) {
    std::move(analysis_buffer_.begin() + meta_.hop_length,
              analysis_buffer_.end(), analysis_buffer_.begin());
    std::copy(hop, hop + meta_.hop_length,
              analysis_buffer_.end() - meta_.hop_length);

    for (int32_t i = 0; i != meta_.window_length; ++i) {
    ...
    Ort::Value spec = Ort::Value::CreateTensor<float>(
        ...
        meta_.spec_shape.data(), meta_.spec_shape.size());
    ...
    std::move(overlap_add_buffer_.begin() + meta_.hop_length,
              overlap_add_buffer_.end(), overlap_add_buffer_.begin());
    std::fill(overlap_add_buffer_.end() - meta_.hop_length,
    ...
    output->insert(output->end(), overlap_add_buffer_.begin(),
                   overlap_add_buffer_.begin() + meta_.hop_length);
  }

 private:
  OnlineSpeechDenoiserConfig config_;
