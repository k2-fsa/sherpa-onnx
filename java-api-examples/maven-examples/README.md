...
```bash
java -jar target/sherpa-onnx-maven-example-1.13.6.jar
```
...
mvn package -q
ls -lh target/sherpa-onnx-maven-example-1.13.6.jar

# List native libs in the jar
unzip -l target/sherpa-onnx-maven-example-1.13.6.jar | grep -E "\.so$|\.dylib$|\.dll$"
```
