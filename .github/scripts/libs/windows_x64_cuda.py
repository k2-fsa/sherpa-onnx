...
class OnnxruntimeConfig:
    ...
    def onnxruntime_url_suffix(self) -> str:
        return f"gpu_cuda{self.cuda_version}-{self.onnxruntime_version}"

    @property
    def onnxruntime_dir_name(self) -> str:
        return f"onnxruntime-win-x64-{self.onnxruntime_url_suffix}"

    @property
    def onnxruntime_url(self) -> str:
        return f"https://github.com/microsoft/onnxruntime/releases/download/v{self.onnxruntime_version}/{self.onnxruntime_dir_name}.zip"
    ...
    def dst_suffix(self) -> str:
        return f"cuda-{self.cuda_version}.x-cudnn-9.x-onnxruntime{self.onnxruntime_version}-win-x64-cuda"
...
def main():
    ...
    for build_type, cfg in itertools.product(build_types, onnxruntime_configs):
        ...
        d["onnxruntime_url"] = cfg.onnxruntime_url
