...
for f in $(find "$gopath/pkg/mod/github.com/k2-fsa/" \
  ...
  mod_dir="$(dirname "$f")"
  echo "[replace-sherpa-onnx-go] Replacing in $mod_dir"
  chmod u+w "$f"
  cp -v "$local_file" "$f"
  # Also copy the build_*.go files which contain #cgo LDFLAGS.
  # The published module is self-contained in sherpa_onnx.go, but the local
  # version splits CGO flags into separate build_*.go files.
  repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
  cp -v "$repo_root"/scripts/go/_internal/build_*.go "$mod_dir/" 2>/dev/null || true
  found=1
done
