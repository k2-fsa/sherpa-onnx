...
function process_linux() {
  mkdir -p t
  cd t
  curl -OL https://$HF_MIRROR/csukuangfj/sherpa-onnx-wheels/resolve/main/$SHERPA_ONNX_VERSION/$linux_wheel_filename
...
function process_macos() {
  mkdir -p t
  cd t
  curl -OL https://$HF_MIRROR/csukuangfj/sherpa-onnx-wheels/resolve/main/$SHERPA_ONNX_VERSION/$macos_wheel_filename
