...
class SenseVoiceSmall(nn.Module):
    ...
    def forward(self, x, x_len, prompt):
        """
        Args:
          x: (N, T, C)
          x_len: (N,)
        ...
        x_len += input_query.shape[1]

        encoder_out, encoder_out_len = self.encoder(x, x_len)
        logits = self.ctc.ctc_lo(encoder_out)

        return logits, encoder_out_len
