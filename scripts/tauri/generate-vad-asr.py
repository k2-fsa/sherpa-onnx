...
"""
Generate build artifacts for the sherpa-onnx Tauri VAD+ASR desktop app.
...
Usage:
  ...
  python3 scripts/tauri/generate-vad-asr.py --gen-workflow
"""
...
def gen_build_script(models: List[Model], output_path: Path):
    """Render the Jinja2 template with the given model list."""
    environment = jinja2.Environment()
    template_path = Path(__file__).parent / "build-tauri-vad-asr.sh.in"
...
def gen_workflow(output_path: Path):
    """Generate GitHub Actions workflow following the APK pattern."""
    # Number of shards — adjust to control parallelism
    total_shards = 10
    shard_indices = [str(i) for i in range(total_shards)]

    lines = []
    lines.append("# Auto-generated by generate-vad-asr.py. DO NOT EDIT.")
    lines.append("name: tauri-vad-asr")
    lines.append("")
    lines.append("on:")
    lines.append("  push:")
    lines.append("    branches: [master]")
    lines.append("    paths:")
    lines.append("      - 'tauri-examples/non-streaming-speech-recognition-from-file/**'")
    lines.append("      - '.github/workflows/tauri-vad-asr.yaml'")
    lines.append("      - 'scripts/tauri/**'")
    lines.append("  workflow_dispatch:")
    lines.append("")
    lines.append("concurrency:")
    lines.append("  group: tauri-vad-asr-${{ github.ref }}")
    ...
    lines.append("")
    lines.append("jobs:")
    lines.append("  tauri_vad_asr:")
    lines.append("    runs-on: ${{ matrix.os }}")
    lines.append(f"    name: ${{{{ matrix.os }}}} ${{{{ matrix.index }}}}/{total_shards}")
    lines.append("    strategy:")
    lines.append("      fail-fast: false")
    lines.append("      matrix:")
    lines.append(f"        os: [ubuntu-latest, ubuntu-24.04-arm, macos-13, macos-latest, windows-latest]")
    lines.append(f"        index: {shard_indices}")
    lines.append("")
    lines.append("    steps:")
    ...
    lines.append("")
    lines.append("      - name: Install Tauri CLI")
    lines.append("        run: cargo install tauri-cli")
    lines.append("")
    lines.append("      - name: Install Python dependencies")
    lines.append("        run: python3 -m pip install --upgrade pip jinja2")
    lines.append("        shell: bash")
    lines.append("")
    lines.append("      - name: Generate build script")
    lines.append("        shell: bash")
    lines.append("        run: |")
    lines.append("          cd scripts/tauri")
    lines.append(f"          python3 generate-vad-asr.py --total {total_shards} --index ${{{{ matrix.index }}}}")
    lines.append("          chmod +x build-tauri-vad-asr.sh")
    lines.append("          mv build-tauri-vad-asr.sh ../..")
    lines.append("")
    lines.append("      - name: Build")
    lines.append("        shell: bash")
    lines.append("        run: ./build-tauri-vad-asr.sh")
    lines.append("")
    lines.append("      - name: Upload artifacts")
    lines.append("        uses: actions/upload-artifact@v4")
    lines.append("        with:")
    lines.append("          name: tauri-${{ matrix.os }}-shard-${{ matrix.index }}")
    lines.append("          path: tauri-examples/non-streaming-speech-recognition-from-file/dist/")
...
def get_args():
    ...
    parser.add_argument("--gen-workflow", action="store_true", help="Generate CI workflow")
    parser.add_argument(
        "--total",
        type=int,
        default=1,
