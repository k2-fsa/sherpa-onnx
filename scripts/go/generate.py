...
def parse_golang(target):
    with open(target, 'r') as file:
        content = file.read()
    defines = []
    struct_pattern = re.compile(r'type\s+([A-Z]\w+)\s+struct', re.IGNORECASE | re.DOTALL)
    ...
    struct_pattern = re.compile(r'type\s+([A-Z][^ =]+)\s*=', re.IGNORECASE | re.DOTALL)
    struct_matches = struct_pattern.findall(content)
    for name in struct_matches:
        c_define = {
            'type': 'struct',
        ...
        defines.append(c_define)
    func_pattern = re.compile(r'func\s+([A-Z][^ \(]+)\s*\(', re.IGNORECASE | re.DOTALL)
